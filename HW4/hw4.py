...
inv_label_dict = {k: {n: i for (i,n) in label_dict[k].items()} for k,d in label_dict.items()}
...
cat_cat_data_dict = makeDataDictForCatCat(cat_cat_params_dict , list(label_dict[cat_cat_params_dict['target_name']].values()) , all_data , color_dict, label_dict)
cat_cat_CDS = ColumnDataSource(data=cat_cat_data_dict)

cat_cat_tree = makeTreeForCatCat(cat_cat_params_dict , all_data , label_dict)
cat_cat_background_dict = makeDataDictForBackgroundCatCat(cat_cat_params_dict , cat_cat_tree , all_data , color_dict, label_dict)
cat_cat_background_CDS = ColumnDataSource(data=cat_cat_background_dict)

cat_cat_prec_rec_data_dict = makeDataDictForPrecRec(cat_cat_params_dict , cat_cat_tree , 0 , all_data , color_dict , label_dict)
cat_cat_prec_rec_CDS = ColumnDataSource(data=cat_cat_prec_rec_data_dict)




cat_cat_x_select = Select(options=list(cat_cat_column_names_dict.keys()) , value=list(cat_cat_column_names_dict.keys())[0] , title="X Data")
cat_cat_y_select = Select(options=list(cat_cat_column_names_dict.keys()) , value=list(cat_cat_column_names_dict.keys())[1] , title="Y Data")
cat_cat_target_select = Select(options=list(cat_cat_column_names_dict.keys()) , value=list(cat_cat_column_names_dict.keys())[2] , title="Target Variable")

cat_cat_mult_select = MultiSelect(options=list(label_dict[cat_cat_params_dict['target_name']].values()),value=list(label_dict[cat_cat_params_dict['target_name']].values()),title="Selected Targets")

inv_label_dict_tuple_list = [(k,str(v)) for (k,v) in inv_label_dict[cat_cat_params_dict['target_name']].items()]
cat_cat_prec_rec_val_select = Select(options=list(inv_label_dict[cat_cat_params_dict['target_name']].keys()) , value=list(inv_label_dict[cat_cat_params_dict['target_name']].keys())[0] , title="Prec-Rec Class")
...
def catCatChangeData():
    ...
    cat_cat_CDS.data = makeDataDictForCatCat(cat_cat_params_dict , cat_cat_mult_select.value, all_data , color_dict, label_dict)
    cat_cat_tree = makeTreeForCatCat(cat_cat_params_dict , all_data , label_dict)
    cat_cat_background_CDS.data = makeDataDictForBackgroundCatCat(cat_cat_params_dict , cat_cat_tree , all_data , color_dict, label_dict)
...
def catCatPrecRecChangeData():
    ...
    cat_cat_prec_rec_CDS.data = makeDataDictForPrecRec(cat_cat_params_dict , cat_cat_tree , inv_label_dict[cat_cat_params_dict['target_name']][cat_cat_prec_rec_val_select.value] , all_data , color_dict , label_dict)
    # print("EXIT -------- PrecRecChangeData ---------\n-----------------")

def catCatAxesCallback(attrname,old,new):
    # print("ENTER -------- AxesCallback ---------")
    cat_cat_params_dict['x_name'] = cat_cat_column_names_dict[cat_cat_x_select.value]
    cat_cat_params_dict['y_name'] = cat_cat_column_names_dict[cat_cat_y_select.value]
...
def catCatTargetSelectCallback(attrname,old,new):
    ...
    cat_cat_params_dict['target_name'] = cat_cat_column_names_dict[cat_cat_target_select.value]
    
    # print("\t\tadded_target_name")
    cat_cat_mult_select.options = list(label_dict[cat_cat_column_names_dict[cat_cat_target_select.value]].values())
    cat_cat_mult_select.value = list(label_dict[cat_cat_column_names_dict[cat_cat_target_select.value]].values())

    # print("\t\tchanged_options")

    cat_cat_prec_rec_val_select.options = list(inv_label_dict[cat_cat_params_dict['target_name']].keys())
    cat_cat_prec_rec_val_select.value = list(inv_label_dict[cat_cat_params_dict['target_name']].keys())[0]
...
curdoc().add_root(column(row(cat_cat_figure , cat_cat_prec_rec_figure) , row(cat_cat_options_box , cat_cat_prec_rec_options_box)))
